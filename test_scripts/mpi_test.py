...
def setEnv():
    ...
    os.environ['BASE_CTX_SWITCH_THRESHOLD'] = "80000"
...
def runTest(buffer_sizes,ITER,machines,i):
    ...
    for iter in ITER:
        ...
        for a in buffer_sizes:
            cmd="mpirun -np "+str(machines)+" -f  ../machinefile "+op['nccl_run']+" -b "+str(a)+" -e "+str(a)+" -f 2 -t 8 -g 1 -n 5 -w 2 -c 0  >>"+ op['nccl_rawData'][iter]
            print(cmd)
            os.system(cmd)
            
            os.system("sleep 10s")
            #input("input anyword to continue") 
            cmd="mpirun -np "+str(machines)+" -f  ../machinefile "+op['occl_run']+" -b "+str(a)+" -e "+str(a)+" -f 2 -t 8 -g 1 -n 5 -w 2 -c 0  >>"+ op['occl_rawData'][iter]
            print(cmd)
            os.system(cmd)
            os.system("sleep 10s")
...
def main():
    ...
    buffer_sizes = ["512", "1K", "2K", "4K", "8K", "16K", "32K", "64K",  "128K", "256K", "512K", "1M", "2M","4M","8M","16M","32M"]
    for i in [0,1,2,3,4]:
